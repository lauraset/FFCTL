...
# 2022.4.4 fuse results of feature and decision levels
class UnetCDdiffuse(torch.nn.Module):
    def __init__(
        self,
        encoder_name: str = "resnet34",
        encoder_depth: int = 5,
    ...
    def forward(self, x):
        ...
        features1 = self.encoder(x[:, :self.inchannels, :, :])
        features2 = self.encoder(x[:, self.inchannels:, :, :])
        # 1. difference in feature level
        featurediff = [torch.abs(f1-f2) for f1, f2 in zip(features1, features2)]  # N C H W
        # if self.isaspp:
        #     features[-1] = self.aspp(features[-1]) # apply aspp to the last layer
