...
'''
def dataloader(filepath, split=0.95):
    # INPUT:
    # filepath: files for train
    # split: train/val dataset
    # RETUREN:
...
# add bound
def dataloader_bound(filepath, split=0.95):
    # INPUT:
    # filepath: files for train
    # split: train/val dataset
    # RETUREN:
    ...
    left = glob(join(filepath,'img', 'img*.tif'))
    cls = glob(join(filepath, 'lab', 'lab*.png'))
    bound = glob(join(filepath, 'bound', 'lab*.png'))

    assert len(left)==len(cls)
    assert len(left)==len(bound)
    left.sort()
    cls.sort()
    bound.sort()
    ...
    left_train = left[train]
    cls_train = cls[train]
    bound_train = bound[train]

    left_val = left[val]
    cls_val = cls[val]
    bound_val = bound[val]

    return left_train, cls_train, bound_train, left_val, cls_val, bound_val


# for 6 class: merge grass and tree
def dataloader_bound_6(filepath, split=0.95):
    # INPUT:
    # filepath: files for train
    # split: train/val dataset
    # RETUREN:
    # left_train, right_train, disp_train_L, left_val, right_val, disp_val_L
    left = glob(join(filepath,'img', 'img*.tif'))
    cls = glob(join(filepath, 'lab_6', 'lab*.png'))
    bound = glob(join(filepath, 'bound', 'lab*.png'))
...
# add coarse label
def dataloader_coarse(filepath, split=0.95):
    # INPUT:
    # filepath: files for train
    # split: train/val dataset
    # RETUREN:
    ...
    left = glob(join(filepath,'img', 'img*.tif'))
    cls = glob(join(filepath, 'lab', 'lab*.png'))
    bound = glob(join(filepath, 'lab_coarse', 'lab*.png'))
...
# add bound, tlc
def dataloader_bound_tlc(filepath, split=0.95):
    # INPUT:
    # filepath: files for train
    # split: train/val dataset
    # RETUREN:
    ...
    cls = glob(join(filepath, 'lab', 'lab*.png'))
    bound = glob(join(filepath, 'bound', 'lab*.png'))
    tlc = glob(join(filepath, 'tlc', 'tlc*.tif'))

    assert len(left)==len(cls)
    assert len(left)==len(bound)
    assert len(left)==len(tlc)
    left.sort()
    cls.sort()
    bound.sort()
    tlc.sort()
    ...
    return left_train, tlc_train, cls_train, bound_train , left_val, tlc_val, cls_val, bound_val
